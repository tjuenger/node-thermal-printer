...
module.exports = {
  ...
  init: function(initConfig) {
    ...
    if (initConfig.removeSpecialCharacters === undefined)
      initConfig.removeSpecialCharacters = false;
    if (initConfig.replaceSpecialCharacters === undefined)
      initConfig.replaceSpecialCharacters = true;
    if (initConfig.extraSpecialCharacters)
      config.specialCharacters = mergeObjects(
        config.specialCharacters,
        initConfig.extraSpecialCharacters
      );
  ...
  getInterface: function() {
    if (iface === null) {
      iface = getInterface(printerConfig.interface);
    }
  ...
  execute: function(cb) {
    this.getInterface().execute(buffer, function(err) {
      if (!err) {
        buffer = null;
  ...
  leftRight: function(left, right) {
    ...
    var width =
      printerConfig.width - left.toString().length - right.toString().length;
  ...
  isPrinterConnected: function(exists) {
    this.getInterface().isPrinterConnected(exists);
  },
  ...
  printBarcode: function(data, type, settings) {
    if (printerConfig.type === printerTypes.EPSON) {
      ...
      console.error(
        "Barcode not supported on '" + printerConfig.type + "' yet!"
      );
    }
  },
  ...
  maxiCode: function(data, settings) {
    if (printerConfig.type === printerTypes.EPSON) {
      ...
      console.error(
        "MaxiCode not supported on '" + printerConfig.type + "' yet!"
      );
    }
  },
  ...
  code128: function(data, settings) {
    if (printerConfig.type === printerTypes.STAR) {
      ...
      console.error(
        "Code128 not supported on '" + printerConfig.type + "' yet!"
      );
    }
  },
  ...
  pdf417: function(data, settings) {
    if (printerConfig.type === printerTypes.STAR) {
      ...
      console.error(
        "PDF417 not supported on '" + printerConfig.type + "' yet!"
      );
    }
  },
    ...
    try {
      ...
      if (image.slice(-4) === '.png') {
        if (printerConfig.type === printerTypes.STAR) {
          ...
          console.error(
            "Image print not supported on '" + printerConfig.type + "' yet!"
          );
        }
      } else {
  ...
  printImageBuffer: function(buffer, callback) {
    ...
    png.on('parsed', function() {
      if (printerConfig.type === printerTypes.STAR) {
        star._printImageBufferStar(this.width, this.height, this.data, function(
          response
        ) {
          if (response) append(response);
          callback(response);
  ...
  raw: function(text, cb) {
    this.getInterface().execute(text, function(err) {
      if (!err) {
        if (typeof cb === 'function') {
          cb(null);
        }
